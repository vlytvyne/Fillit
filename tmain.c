...
int		main(void)
{
	...
	//board[0][1] = BLOCK;
	list = ft_block_new();
	list->shape[0] = ft_strdup(".A");
	list->shape[1] = ft_strdup(".A");
	list->shape[2] = ft_strdup("AA");
	list->shape[3] = 0;
	list->height = 3;
	list->width = 2;
	list->next = ft_block_new();
	list->next->shape[0] = ft_strdup("BB");
	list->next->shape[1] = ft_strdup("BB");
	list->next->shape[2] = 0;
	list->next->height = 2;
	list->next->width = 2;
	list->next->next = ft_block_new();
	list->next->next->shape[0] = ft_strdup("CCC");
	list->next->next->shape[1] = ft_strdup(".C.");
